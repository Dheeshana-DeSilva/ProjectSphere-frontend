...
import { 
  ...
  Rocket, ExternalLink, FileText, Sparkles, Wrench, Target 
} from 'lucide-react';
...
import { MOCK_PROJECTS_MAP as MOCK_PROJECTS } from '../../data/mockProjects';
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden">
      ...
      <div className="h-48 bg-gradient-to-br from-blue-50 to-slate-100 flex flex-col items-center justify-center gap-2 border-b border-slate-200">
        ...
        <span className="text-xs text-slate-400 font-medium">{diagram.label}</span>
        <span className="text-xs text-slate-300">Diagram image will appear here</span>
      </div>
      <div className="p-4">
        <p className="font-semibold text-slate-700 text-sm mb-1">{diagram.label}</p>
        <p className="text-slate-500 text-xs leading-relaxed">{diagram.description}</p>
      </div>
    </div>
  );
}
